...
- **Write-path errors now preserve InfluxDB's error body.**
  ...
  A duplicate-tag-key write (rejected by InfluxDB 3.11+ before it reaches storage) now reaches the model naming the tag.
...
- **Added a 503 arm to the write error handler**, phrased as retryable (`Service temporarily unavailable, retry the write: ...`), so a model can distinguish a transient failure from a bad request.
